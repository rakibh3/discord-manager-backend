...
Express 5 + TypeScript + Prisma 7 (PostgreSQL) REST API for a Discord daily-attendance / update automation admin dashboard. Authentication is strictly for administrators (no regular user login/registration; students/members submit attendance directly via web form). Auth, user, the Discord bot / member-sync modules, the public attendance endpoints, `#daily-update` message ingestion, the admin-managed channel open/lock scheduler, the rate-limited reminder DM queue, and the scheduled `#attendance` announcement exist. Still missing from the attendance domain: the dashboard endpoints (including the SSE progress stream that wraps the reminder progress read).
...
`src/modules/attendance/` holds the **only three routes with no `auth()` middleware**: `GET /api/attendance/verify-user?username=…`, `POST /api/attendance/submit`, and `GET /api/attendance/window`. That is not an oversight — students are not `users` rows, so the form has no credential to present. Anything added to `attendanceRouter` inherits that exposure.

What stands in for authentication:

- **The membership check (for verify-user and submit).** The handle must resolve to a member with `isInGuild: true`. `verify-user` is a UI affordance for the form's badge; **`submit` re-runs normalization, format validation, and the lookup itself** and never trusts that verify was called. Nothing forces a client to call it, and a member can leave the guild between the two requests. Golden Rule 3 is enforced on the write path, not in the browser. Both endpoints resolve the member through one shared service helper so the two definitions of "verified" cannot drift.
- **Schedule projection (for window).** `GET /api/attendance/window` exposes no member data; `isOpen` is computed from the stored schedule row and Asia/Dhaka clock. It must **never** perform a live Discord channel read (`isDailyUpdateChannelOpen()`), because this endpoint is hit on every student page load — thousands of times in an evening — and an exhausted Discord rate limit would take down member sync and form verification together (violating Golden Rule 4).
- **`src/middlewares/rateLimit.ts`.** Per-IP budgets — 60/min on `window` and `verify-user` (cheap reads), 5/15min on `submit` (a legitimate student submits once a day). The store is `express-rate-limit`'s in-memory default, so counters are **process-local**: N processes multiply the effective budget by N. Redis now exists in the project (for the reminder queue) but these limiters were **deliberately left on the in-memory store** — see the reminder-queue section for why the two systems fail in opposite directions. The swap remains a one-file change.
...
### Attendance announcement (`#attendance`)

The evening message telling students to submit. `announcement_templates` holds one row (`key = 'ATTENDANCE_DAILY'`) with the body, the mention allowlist, and its own schedule; `src/utils/announcementTemplate.ts` is the pure renderer; `src/lib/discord/announcement.ts` is the **only** module that writes to the attendance channel; `src/lib/announcement/announcement.dispatch.ts` orchestrates; `src/lib/scheduler/announcement.scheduler.ts` holds the cron task; `src/modules/announcement/` exposes it at `/api/announcement` (all routes `auth(ADMIN)`).

- **Mentions come only from the structured allowlist, never from the body.** `allowedMentions` is `{ parse: mentionEveryone ? ['everyone'] : [], roles, users }`, built from `mentionRoleIds` / `mentionUsernames` / `mentionEveryone`. `parse: []` is the load-bearing part — the same guarantee `announceClosedDms` relies on — and it makes an `@everyone` typed into the Bangla body inert no matter what the text says. A body is admin-editable free text posted to ~5,000 people; parsing mentions out of it would make every edit a potential mass ping.
- **`mentionEveryone` is its own boolean**, off by default, audited via `updatedById`. It cannot be an entry in `mentionRoleIds`, because Discord's `@everyone` role ID _is_ the guild ID — an admin discovering that in a free-text field is an accident waiting to happen.
- **The body is Discord markdown, sent verbatim.** Nothing between `announcement_templates.body` and `channel.send()` escapes or strips it, so `#`/`##`/`###` headings, `**bold**`, `-` lists and fenced code blocks render as formatting. The default body uses them throughout: `#` title, `###` date line, `##` section headings, `**bold**` on the closing time and the termination threshold, and a bulleted list with bold labels for the four update fields. Markdown changes nothing about notifications — `allowedMentions` still names every ping, so an `@everyone` typed into a heading stays as inert as one in a paragraph. The length check is measured on this rendered text, so markdown syntax counts against the 2,000 limit.
- **Plain `content`, never an embed.** Mentions inside an embed do not notify anybody; Discord resolves pings only in `content`. The open/lock notices are embeds because they mention no one. Getting this wrong produces a message that looks right and reaches nobody.
- **The mention line is appended, not substituted.** There is deliberately no `{{mentions}}` placeholder — it would let an admin delete the mentions by editing the text.
- **Only the five supported placeholders are accepted**, rejected on save by `findUnsupportedPlaceholders`. That check raises `AppError` in `announcement.service.ts` rather than living in the Zod schema, because `handleZodValidationError` title-cases every word and would hand the admin `{{Close_time}}` — a message whose whole purpose is naming the exact token to type. The rendered-length check against Discord's 2,000 limit is measured on the **rendered** output plus the mention line, since expansion is what closes the gap.
- **`{{close_time}}` is read from `channel_schedules`, never stored twice.** The two features share that one value and no control: it is why the message can never state a closing time different from the one that actually locks `#daily-update`.
- **One post per Dhaka day, claimed in the database.** `@@unique([key, announcementDate, attempt])`; the claim is a `create` of a `SENDING` row and P2002 means the day is taken. A `FAILED` row is re-taken through `reclaimFailedDay()`, an `updateMany` scoped to `status: 'FAILED'` — the same scoped-claim trick as `markReminderProcessing`, and the reason a failed send does not consume the day. `force: true` claims `nextAttemptNumber()` instead.
- **The DB claim alone does not guarantee one message — `enforceNonce` does.** The claim guarantees `channel.send()` is _called_ once a day; it cannot see the HTTP layer beneath it. discord.js's REST client retries a request that times out, and a POST that Discord already processed but whose response was lost is indistinguishable from one it never received — so each retry creates another identical message while only the last response returns a message ID. The log then shows one post and the channel shows several (observed: four messages from one send, 53 seconds between the claim row and the success log). Every send therefore carries `enforceNonce: true` with a deterministic `nonce` of `<announcementDate>-<attempt>`; Discord matches it against recent messages from the same author and returns the existing one instead of creating a second. The two mechanisms cover different windows — the nonce covers one call's retries, the claim covers the day — and neither replaces the other.
- **The claim is taken BEFORE the send, deliberately.** Recording after a successful post leaves a window where a crash between the two lets the next run post a duplicate. Claiming first inverts the risk into "claimed but never posted", which is visible as a `SENDING` row in `GET /api/announcement/attendance` and is recoverable with a forced manual send. A duplicate mass-mention in a channel the whole program reads is not recoverable.
- **Mention targets are validated on save, resolved at post time.** A member can leave and a role can be deleted between the save and 7 PM; an unresolved entry is dropped from that post and recorded in `unresolvedTargets`, never a reason to withhold the message. Handles resolve through `memberRepository.findActiveMemberByUsername`, so "who counts as a member" keeps one definition.
- **The schedule is genuinely independent of the channel window.** Its own row, its own cron task, its own enabled flag. Disabling either leaves the other running; both default to firing in the same evening hour and share nothing but `closeTime`.
- **`SCHEDULER_ENABLED` gates this task too** — one flag for both cron features, since it is the same process-local constraint. The manual send and the status read work on every replica.
- **There is deliberately no boot reconcile**, unlike `startChannelScheduler()`. A locked channel at 8 PM is a state to correct; a missed announcement is a moment that passed, and posting "today's announcement" at 21:40 because a container restarted then — five times during a deploy — is worse than the gap. `today.posted` reports it and the manual send fixes it.
- **`destroy()` the task on reload, never `stop()`**, for the same reason as the channel scheduler.
- **The bot needs `Send Messages` on the attendance channel.** Without it every send fails and the channel simply goes quiet — no other symptom. Reported as a 403 on send and as `scheduler.lastOutcome` on the read; check it there first.
- The announcement is posted by the bot, so the existing bot-author filter in ingestion excludes it — no `daily_updates` row is ever created for it.
- Nothing in `announcement.ts`, `announcement.dispatch.ts`, or the scheduler throws past its own boundary. `AppError` appears only in `announcement.service.ts`.
