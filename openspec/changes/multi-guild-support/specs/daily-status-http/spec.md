...
- **THEN** each row carries `memberId`, `guildId`, `serverLabel`, `serverCount`, `discordUserId`, `discordUsername`, `displayName`, `name`, `email`, `phone`, `hasAttendance`, `hasDailyUpdate`, `status`, and `attendanceSubmittedAt` (ISO string or null)
...
- **THEN** two rows are returned, one per server, each with its own `guildId` and its own status
- **AND** each row reports `serverCount` greater than one

### Requirement: Single member daily status is served over an admin-only HTTP endpoint

The system SHALL expose `GET /api/daily-status/members/:memberId` as an admin-only endpoint that returns one member record's status for a date, plus that day's daily-update messages. The member ID SHALL identify one member record, and therefore one server.
...
- **THEN** the response is 200 with the member's row fields, including `guildId` and `serverLabel`, plus a `messages` array where each message has `id`, `content`, and `postedAt` (ISO string)
...
- **THEN** only the requested record's status and messages are returned
...
The system SHALL expose `GET /api/daily-status/export` as an admin-only endpoint that returns a downloadable file (CSV or XLSX) of the filtered member status for a date, honouring the same optional `guildId` filter as the list endpoint and naming the server on every row.
...
- **AND** columns are `server`, `discordUsername`, `displayName`, `name`, `phone`, `email`, `status`, `hasAttendance`, `hasDailyUpdate`, `attendanceSubmittedAt`

#### Scenario: Export honours the server filter

- **WHEN** the export request includes `guildId=` a configured server
- **THEN** only that server's rows appear in the exported file
