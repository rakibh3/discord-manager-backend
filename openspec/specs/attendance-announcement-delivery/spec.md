# attendance-announcement-delivery Specification

## Purpose

Defines how the shared attendance announcement is actually posted: the per-server channel lookup, the once-per-day claim, and the per-server outcome reported to administrators. The body and schedule are shared; the post is per server.
