# public-attendance-window Specification

## Purpose

Exposes an unauthenticated, public projection of the submission window for the web attendance form. The window is derived entirely from the stored channel schedule and the Asia/Dhaka clock, without calling external Discord APIs. The response provides students with the schedule hours, current open/closed status, next opening instant, and closing instant while strictly withholding any administrative details (editor identity, scheduler run state, channel ID).
