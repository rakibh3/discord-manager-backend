# daily-status-http Specification

## Purpose

Defines the HTTP surface of the daily-status feature that exposes summary counts, paginated member tables, single-member detail with daily-update message history, and filtered CSV/XLSX export to the admin dashboard. All endpoints are admin-authenticated. Field names match the frontend contract (`bothComplete`, `attendanceSubmittedAt`), and BigInt database figures are safely serialized as JSON numbers.

A Discord account may be a member of more than one configured server, so the surface carries the per-server view alongside the account-level view: counts include a `byServer` breakdown, the page returns one row per account with the servers it belongs to, and the configured servers themselves are discoverable so a dashboard can build a server filter without hard-coding identifiers.
