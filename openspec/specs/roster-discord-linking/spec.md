# roster-discord-linking Specification

## Purpose

Records the Discord account an enrolled person submits attendance from, so the roster report can answer "which enrolled people are doing the work" rather than "which Discord accounts are active". The pairing is the bridge between the enrolment roll (keyed on email) and the daily-status aggregation (keyed on the Discord account), and it is the smallest data structure that lets one query ask both. The pairing is a record, not a permission: a recorded pairing gates nothing, and a missing pairing gates nothing — both are normal states that the roster report distinguishes.
