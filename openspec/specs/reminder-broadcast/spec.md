# reminder-broadcast Specification
...
Starting a broadcast is an acknowledgement, not a completion — the session and every recipient record are written first, delivery is enqueued, and the response carries the broadcast's identifier and target count without claiming any DM has been sent. Two broadcasts for one date may not run at once, because a repeated click must not schedule a second mass DM behind the first, and a finished broadcast's counts are recomputed from its recipient records rather than trusted from the running counters. Everything here is administrator-only: unlike the attendance form there is no public surface in this capability, and a mass DM to thousands of students is not something an unauthenticated caller may reach.
