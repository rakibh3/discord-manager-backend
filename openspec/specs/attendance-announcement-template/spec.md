# attendance-announcement-template Specification

## Purpose

Defines the single shared announcement template, schedule, and mention allowlist that drive the attendance announcement on every configured server. The template is stored once; the post is resolved per server.
