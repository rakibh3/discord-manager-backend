...
The schedule is shared across every configured server. It is stored once and applied to every server's daily-update channel; per-server overrides are not possible, so the channels cannot drift apart. The status read reflects that by reporting every server the schedule applies to and each server's live channel state, not just the stored times.
